...
*   **🟢 我们的方案**:
    ...
    *   **资源泄漏修复**: 在 `hook.ts` 中增加异步竞态检查 (Race Condition Check)。
        *   **问题**: React Strict Mode 或快速操作下，`getUserMedia` 返回时组件已卸载，导致 Stream 变成“幽灵流”无法关闭。
        *   **修复**: 在 `getUserMedia` 返回后立即检查 `hasStoppedRef`，若已停止则立即销毁流。
    *   **编辑器缓存修复**: 修复了纯文本笔记无法清除本地草稿的问题。
        *   **问题**: `onSend` 回调中，文本缓存的清除逻辑错误地依赖于“附件缓存存在”。若无附件，文本草稿将残留，导致下次编辑时显示旧内容。
        *   **修复**: 解耦清除逻辑，强制独立检查并清除文本和附件的 `localStorage` 缓存。
