...
### 11. AI 体验优化 (Phase 11 - AI UX)
*   **AI 智能轮询 (Smart Polling)**:
    *   **无感更新**: 在用户提交笔记后，前端检测到开启了 AI 后处理时，会自动启动静默轮询 (Smart Polling)。一旦后端生成了 AI 标签，即可**原地更新** UI，无需用户手动刷新。
    *   **安全锁 (Safety Guard)**: 引入了“编辑态检测”，若用户正在编辑该笔记的草稿，轮询器会自动终止，杜绝了“AI 更新覆盖用户输入”的风险。
*   **智能上下文 (Smart Context)**:
    *   **标签排重 (Leaf-Only Logic)**: 重写了后端 `getAllPathTags` 逻辑。针对 AI 提示词上下文，**自动过滤掉父级路径**（如 `#Work/Project` 存在时，不再输出 `#Work`）。
    *   **效果**: 彻底解决了 AI 在生成标签时倾向于输出冗余父级标签的问题，显著提升了自动分类的精准度。

---

### 12. 删除与稳定性专项 (Phase 12 - Deletion & Stability)
*   **回收站性能重构 (Trash V3)**:
    *   **并发分块**: 将串行删除重构为 **并发分块处理 (Chunked Parallel)**，每批次并发处理 5 条记录。
    *   **向量库批量删除**: 引入 `queryAndDeleteVectorByIds`，将 N+1 次数据库调用压缩为 **1次** 批量操作，彻底解决大批量清空时的超时问题。
    *   **超时熔断**: 为向量库清理增加 8秒 强制超时保护，确保主流程不被阻塞。
*   **交互体验大升级**:
    *   **瞬间离场**: 优化 `removeLocalNote` 覆盖回收站与归档列表，点击删除即刻从 UI 消失。
    *   **AI 通知优化**: AI 生成空标签时明确提示 "暂无 AI 标签"，消除用户困惑。
*   **核心修复**:
    *   **UseEditor 内存泄漏**: 修复了 `eventBus` 监听器未解绑导致的 `MaxListenersExceededWarning`。
    *   **P2025 异常**: 修复了附件删除时的竞争条件 (Record not found)，允许忽略已删除记录的错误。
*   **插件系统与异步修复**:
    *   **路径兼容性**: 修复了 `plugin.ts` 在生产与开发环境下路径解析不一致的问题。
    *   **安装校验**: 为插件安装接口增加了 `url` 必填校验，防止空指针异常。
    *   **UI 异步**: 修复了 `cardBlogBox` 删除操作未等待 Promise 完成的问题。