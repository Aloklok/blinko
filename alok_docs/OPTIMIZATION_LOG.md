...
*   **卡片图片显示与缩放**: 修复了卡片中图片显示不出的问题，并随后解决了缩放尺寸回归的问题（Phase 42 & 44）。通过以下方式优化：1. 修正 `HandleFileType` 状态逻辑；2. 恢复卡片预览图为 100x100px 固定尺寸；3. 将 `object-cover` 切换为 `object-fill`（拉伸全显模式），确保多图排版整齐且不被裁剪。
