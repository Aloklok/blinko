...
## 10. 图标系统异步化重构 (Icon System Restructuring)

为了解决官方单体 `icons.tsx` 随图标增多而膨胀的问题，我们将图标加载逻辑改造成了**分集合动态加载架构**。

*   **构建逻辑 (`buildIcons.js`)**: 自动扫描图标使用情况，并按集合（如 `solar`, `eva`, `lucide`）将图标元数据拆分到 `app/src/components/Common/Iconify/collections/` 下的独立文件中。
*   **渲染逻辑 (`icons.tsx`)**: 
    *   采用 **Collection Proxy** 模式，只有当特定前缀的图标被请求时，才会动态加载对应的集合文件。
    *   **平滑回退**: 引入了 `fallback` 机制，若本地集合未命中，会自动切换至 `@iconify/react` 在线模式，兼顾性能与灵活性。
