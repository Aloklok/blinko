...
## 15. iOS Safari 音频格式兼容性 (Audio Format Compatibility)

解决了 Mac 录制的音频文件无法在 iOS Safari 中播放的问题。

### 15.1 跨平台播放失败问题
*   **🔴 问题**: Mac Safari 录制的音频文件（WebM 格式）无法在 iOS Safari 中播放，控制台报错 `Audio loading error` 和 `Voice playback error`。
*   **分析**: 
    1.  Mac Safari 优先使用 `audio/webm;codecs=opus` 格式录制。
    2.  iOS Safari 虽然 MediaRecorder 支持录制 WebM，但**播放 WebM 的支持有限**。
    3.  iOS Safari 自身录制时会回退到 `audio/mp4`，因此 iOS 录制的文件可以正常播放。
*   **🟢 方案**: 修改 `AudioRecorder/hook.ts`，将 MIME 类型优先级调整为 `audio/mp4` 优先。
*   **收益**: 
    *   **跨平台兼容**: Mac/iOS/Chrome 录制的文件均可在所有平台播放。
    *   **无需转码**: 避免服务端转码的额外成本和延迟。

---

## 📌 已知问题与待办项目 (Known Issues / TODO)

### 16.1 Safari 15.x 正则解析错误
